...
func _ready() -> void:
	...
	add_to_group("ApothecaryShops")
	add_to_group("MarketStall")
	add_to_group("nav_carve_obstacles")
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Apothecary Shop")
	...
	# Load Apothecary recipes inside the bench
	var bench = get_node_or_null("CraftingBench")
	if bench:
		bench.bench_name = "Apothecary Compounding Bench"
		bench.recipes.clear()
		var r1 = load("res://common/items/recipes/crush_root_pigment.tres")
		var r2 = load("res://common/items/recipes/brew_stamina_draught.tres")
		var r3 = load("res://common/items/recipes/mix_wood_varnish.tres")
		var r4 = load("res://common/items/recipes/distill_concentrated_dye.tres")
		var r5 = load("res://common/items/recipes/mix_nitre_compound.tres")
		var r6 = load("res://common/items/recipes/refine_soothing_salve.tres")
