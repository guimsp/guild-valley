...
func _ready() -> void:
	...
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Imperial Gastronomy Spire")
	...
	# Load standard Spire recipes inside the bench
	var bench = get_node_or_null("CraftingBench")
	if bench:
		bench.bench_name = "Imperial Gastronomy Bench"
		bench.recipes.clear()
		var r1 = load("res://common/items/recipes/sovereign_ambush_banquet.tres")
		var r2 = load("res://common/items/recipes/vintage_sovereign_nectar.tres")
