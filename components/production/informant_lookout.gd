...
func _ready() -> void:
	...
	add_to_group("InformantLookouts")
	add_to_group("MarketStall")
	add_to_group("nav_carve_obstacles")
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Informant Lookout")
	
	# Trigger redraw of map to instantly strip fog-of-war attributes
	var map_graphics = get_tree().get_first_node_in_group("map_graphics")
	if map_graphics:
		map_graphics.queue_redraw()
	...
	# Load Informant Lookout recipes inside the bench
	var bench = get_node_or_null("CraftingBench")
	if bench:
		bench.bench_name = "Lookout Table"
		bench.recipes.clear()
		var r1 = load("res://common/items/recipes/compile_ledger_report.tres")
