...
func _ready() -> void:
	...
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Loom")
	...
	# Load Tailor weaving recipe inside the bench
	var bench = get_node_or_null("CraftingBench")
	if bench:
		bench.bench_name = "Loom"
		var weave_cloth = load("res://common/items/recipes/weave_cloth.tres")
		bench.recipes.clear()
		if weave_cloth:
			bench.recipes.append(weave_cloth)
