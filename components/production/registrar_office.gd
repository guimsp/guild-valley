...
func _ready() -> void:
	...
	add_to_group("RegistrarOffices")
	add_to_group("MarketStall")
	add_to_group("nav_carve_obstacles")
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Registrar Office")
	...
	# Load Registrar Office recipes inside the bench
	var bench = get_node_or_null("CraftingBench")
	if bench:
		bench.bench_name = "Registrar Desk"
		bench.recipes.clear()
		
		var r_tax = load("res://common/items/recipes/local_tax_assessment.tres")
		var r_lic = load("res://common/items/recipes/corporate_licensing_desk.tres")
		var r_writ = load("res://common/items/recipes/seal_tax_exemption_writ.tres")
		var r_charter = load("res://common/items/recipes/craft_imperial_trade_charter.tres")
		
		if r_tax: bench.recipes.append(r_tax)
		if r_lic: bench.recipes.append(r_lic)
		if r_writ: bench.recipes.append(r_writ)
		if r_charter: bench.recipes.append(r_charter)
