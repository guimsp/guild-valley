...
func _ready() -> void:
	...
	add_to_group("CarpentryWorkshops")
	add_to_group("MarketStall")
	add_to_group("nav_carve_obstacles")
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Carpentry Workshop")
	...
	# Load Carpentry Workshop recipes inside the bench
	var bench = get_node_or_null("CraftingBench")
	if bench:
		bench.bench_name = "Carpenter's Bench"
		bench.recipes.clear()
		
		# Level 1 Recipes
		var r1 = load("res://common/items/recipes/assemble_basic_crate.tres")
		var r2 = load("res://common/items/recipes/carve_tool_handles.tres")
		if r1: bench.recipes.append(r1)
		if r2: bench.recipes.append(r2)
		
		# Level 2 Recipes
		if building_level >= 2:
			var r3 = load("res://common/items/recipes/craft_cargo_keg.tres")
			var r4 = load("res://common/items/recipes/build_loom_frame.tres")
			if r3: bench.recipes.append(r3)
			if r4: bench.recipes.append(r4)
			
		# Level 3 Recipes
		if building_level >= 3:
			var r5 = load("res://common/items/recipes/construct_shipping_crate.tres")
			if r5: bench.recipes.append(r5)
			
		# Level 4 Recipes
		if building_level >= 4:
			var r6 = load("res://common/items/recipes/carve_ornate_fittings.tres")
