...
func _ready() -> void:
	# Call parent base ready setup (initializes inventories, strongbox, doors, interior space)
	_ready_base()
	
	add_to_group("Bakeries")
	add_to_group("MarketStall")
	add_to_group("nav_carve_obstacles")
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Bakery")
	...
	# Load standard patreon bakery recipes inside the bench
	var bench = get_node_or_null("CraftingBench")
	if bench:
		bench.bench_name = "Bakery"
		bench.recipes.clear()
		
		# L1 Recipes
		var r1 = load("res://common/items/recipes/bake_bread.tres")
		var r2 = load("res://common/items/recipes/bake_eggs.tres")
		var r3 = load("res://common/items/recipes/bake_apples.tres")
		if r1: bench.recipes.append(r1)
		if r2: bench.recipes.append(r2)
		if r3: bench.recipes.append(r3)
		
		# L2 Recipes (Sweet berry cake, Apothecary's sweet bun)
		if building_level >= 2:
			var r4 = load("res://common/items/recipes/bake_berry_cake.tres")
			var r5 = load("res://common/items/recipes/bake_sweet_bun.tres")
			if r4: bench.recipes.append(r4)
			if r5: bench.recipes.append(r5)
			
		# L3 Recipes (Gilded Cream Cake, Royal Venison Pasty)
		if building_level >= 3:
			var r6 = load("res://common/items/recipes/bake_cream_cake.tres")
			var r7 = load("res://common/items/recipes/bake_venison_pasty.tres")
