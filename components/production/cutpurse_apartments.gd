...
func _ready() -> void:
	...
	add_to_group("CutpurseApartments")
	add_to_group("MarketStall")
	add_to_group("nav_carve_obstacles")
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Cutpurse Apartments")
	...
	# Load Cutpurse Apartments recipes inside the bench
	var bench = get_node_or_null("CraftingBench")
	if bench:
		bench.bench_name = "Apartment Desk"
		bench.recipes.clear()
		var r1 = load("res://common/items/recipes/commercial_protection_desk.tres")
