...
func _ready() -> void:
	...
	add_to_group("PalaceSpires")
	add_to_group("MarketStall")
	add_to_group("nav_carve_obstacles")
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Black Market Palace Spire")
	...
	# Load Spire recipes inside the bench
	var bench = get_node_or_null("CraftingBench")
	if bench:
		bench.bench_name = "Black Market Palace Workbench"
		bench.recipes.clear()
		var r1 = load("res://common/items/recipes/forge_extortion_mandate.tres")
		var r2 = load("res://common/items/recipes/forge_underworld_safe_conduct.tres")
