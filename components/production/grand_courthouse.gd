...
func _ready() -> void:
	...
	add_to_group("Courthouses")
	add_to_group("MarketStall")
	add_to_group("nav_carve_obstacles")
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Grand Courthouse")
	...
	# Load Courthouse recipes inside the bench
	var bench = get_node_or_null("CraftingBench")
	if bench:
		bench.bench_name = "Courthouse Docket"
		bench.recipes.clear()
		
		var r_estate = load("res://common/items/recipes/contract_liquidate_defaulted_estate.tres")
		var r_defense = load("res://common/items/recipes/contract_royal_monopoly_defense.tres")
		
		if r_estate: bench.recipes.append(r_estate)
		if r_defense: bench.recipes.append(r_defense)
