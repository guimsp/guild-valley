...
func _ready() -> void:
	...
	add_to_group("PrintingPresses")
	add_to_group("MarketStall")
	add_to_group("nav_carve_obstacles")
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Printing Press")
	...
	# Load standard Printing Press recipe inside the bench
	var bench = get_node_or_null("CraftingBench")
	if bench:
		bench.bench_name = "Printing Press"
		var print_book = load("res://common/items/recipes/print_book.tres")
		bench.recipes.clear()
		if print_book:
			bench.recipes.append(print_book)
