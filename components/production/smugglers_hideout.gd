...
func _ready() -> void:
	...
	add_to_group("SmugglersHideouts")
	add_to_group("MarketStall")
	add_to_group("nav_carve_obstacles")
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Smuggler's Hideout")
	...
	# Load Smuggler's Hideout recipes inside the bench
	var bench = get_node_or_null("CraftingBench")
	if bench:
		bench.bench_name = "Hideout Workbench"
		bench.recipes.clear()
		var r1 = load("res://common/items/recipes/melt_salvaged_scrap.tres")
		var r2 = load("res://common/items/recipes/carve_bone_buttons.tres")
		var r3 = load("res://common/items/recipes/peel_willow_shoots.tres")
