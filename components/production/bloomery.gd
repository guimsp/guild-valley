...
func _ready() -> void:
	...
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Bloomery Smelter")
	...
	# Load Bloomery recipes inside the bench
	var bench = get_node_or_null("CraftingBench")
	if bench:
		bench.bench_name = "Bloomery Hearth"
		bench.recipes.clear()
		var r1 = load("res://common/items/recipes/smelt_iron.tres")
		var r2 = load("res://common/items/recipes/cast_iron_nails.tres")
		var r3 = load("res://common/items/recipes/roll_sheet_metal.tres")
