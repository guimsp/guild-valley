...
func _ready() -> void:
	...
	add_to_group("RoyalOperaHouses")
	add_to_group("MarketStall")
	add_to_group("nav_carve_obstacles")
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Royal Opera House")
	...
	if bench:
		bench.bench_name = "Royal Opera House Bench"
		bench.recipes.clear()
		var recipes_to_load = [
			"res://common/items/recipes/construct_acoustic_domes.tres"
		]
