...
func _ready() -> void:
	...
	add_to_group("ShadowBrokersRings")
	add_to_group("MarketStall")
	add_to_group("nav_carve_obstacles")
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Shadow Broker's Ring")
	...
	# Load Shadow Broker recipes inside the bench
	var bench = get_node_or_null("CraftingBench")
	if bench:
		bench.bench_name = "Shadow Broker's Table"
		bench.recipes.clear()
