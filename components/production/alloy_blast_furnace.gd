...
func _ready() -> void:
	...
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Alloy Blast Furnace")
	...
	# Load Blast Furnace recipes inside the bench
	var bench = get_node_or_null("CraftingBench")
	if bench:
		bench.bench_name = "Blast Furnace"
		bench.recipes.clear()
		var r1 = load("res://common/items/recipes/refine_carbon_steel.tres")
		var r2 = load("res://common/items/recipes/smelt_brass_alloy.tres")
