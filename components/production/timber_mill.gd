...
func _ready() -> void:
	...
	add_to_group("TimberMills")
	add_to_group("MarketStall")
	add_to_group("nav_carve_obstacles")
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Timber Mill")
	...
	# Load Timber Mill recipes inside the bench
	var bench = get_node_or_null("CraftingBench")
	if bench:
		bench.bench_name = "Timber Mill Saw"
		bench.recipes.clear()
		var r1 = load("res://common/items/recipes/saw_timber.tres")
		var r2 = load("res://common/items/recipes/chop_firewood.tres")
		var r3 = load("res://common/items/recipes/shave_wooden_pegs.tres")
