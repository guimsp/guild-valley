...
func _ready() -> void:
	...
	add_to_group("GrandAmphitheaters")
	add_to_group("MarketStall")
	add_to_group("nav_carve_obstacles")
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Grand Amphitheater")
	...
	if bench:
		bench.bench_name = "Grand Amphitheater Bench"
		bench.recipes.clear()
		var recipes_to_load = [
			"res://common/items/recipes/compose_masterpiece_opera.tres"
		]
