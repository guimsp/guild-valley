...
func _ready() -> void:
	...
	add_to_group("ScenicDesignLofts")
	add_to_group("MarketStall")
	add_to_group("nav_carve_obstacles")
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Scenic Design Loft")
	...
	if bench:
		bench.bench_name = "Scenic Design Loft Bench"
		bench.recipes.clear()
		var recipes_to_load = [
			"res://common/items/recipes/paint_scenic_backdrop.tres",
			"res://common/items/recipes/tailor_royal_regalia.tres",
			"res://common/items/recipes/construct_grand_stage_set.tres"
		]
