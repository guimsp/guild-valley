...
func _ready() -> void:
	...
	add_to_group("ArtisanAteliers")
	add_to_group("MarketStall")
	add_to_group("nav_carve_obstacles")
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Artisan Atelier")
	...
	# Load recipes inside the bench
	var bench = get_node_or_null("CraftingBench")
	if bench:
		bench.bench_name = "Artisan Atelier Bench"
		bench.recipes.clear()
		var recipes_to_load = [
			"res://common/items/recipes/grind_base_pigments.tres",
			"res://common/items/recipes/weave_basic_canvas.tres",
			"res://common/items/recipes/refine_pottery_clay.tres",
			"res://common/items/recipes/paint_landscape.tres",
			"res://common/items/recipes/sculpt_clay_flute.tres",
			"res://common/items/recipes/mold_festival_mask.tres",
			"res://common/items/recipes/assemble_stage_costume_showman.tres"
		]
		for r_path in recipes_to_load:
			var r = load(r_path)
			if r:
				bench.recipes.append(r)
	...
	# Create interaction areas dynamically
	_create_dynamic_interaction_areas()

func _create_dynamic_interaction_areas() -> void:
