...
func _ready() -> void:
	...
	add_to_group("ArchitectureAteliers")
	add_to_group("MarketStall")
	add_to_group("nav_carve_obstacles")
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Architecture Atelier")
	...
	# Load Architecture Atelier recipes inside the bench
	var bench = get_node_or_null("CraftingBench")
	if bench:
		bench.bench_name = "Drafting Table"
		bench.recipes.clear()
		
		# Level 1 Recipes
		var r1 = load("res://common/items/recipes/mill_advanced_beam.tres")
		var r2 = load("res://common/items/recipes/frame_modular_wing.tres")
		if r1: bench.recipes.append(r1)
		if r2: bench.recipes.append(r2)
			
		# Level 2 Recipes
		if building_level >= 2:
			var r3 = load("res://common/items/recipes/carve_heavy_scaffolding.tres")
