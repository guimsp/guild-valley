...
func _ready() -> void:
	...
	add_to_group("CitadelSpires")
	add_to_group("MarketStall")
	add_to_group("nav_carve_obstacles")
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Citadel Engineering Spire")
	...
	if bench:
		bench.bench_name = "Monumental Framing Rig"
		bench.recipes.clear()
		var r1 = load("res://common/items/recipes/forge_vault_door.tres")
		var r2 = load("res://common/items/recipes/erect_monumental_trusses.tres")
