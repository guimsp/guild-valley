...
func _ready() -> void:
	...
	add_to_group("HardwoodKilns")
	add_to_group("MarketStall")
	add_to_group("nav_carve_obstacles")
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Hardwood Kiln")
	...
	# Load Hardwood Kiln recipes inside the bench
	var bench = get_node_or_null("CraftingBench")
	if bench:
		bench.bench_name = "Hardwood Curing Kiln"
		bench.recipes.clear()
		var r1 = load("res://common/items/recipes/cure_hardwood.tres")
		var r2 = load("res://common/items/recipes/burn_industrial_charcoal.tres")
