...
func _ready() -> void:
	...
	add_to_group("CrimeSyndicates")
	add_to_group("MarketStall")
	add_to_group("nav_carve_obstacles")
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Crime Syndicate HQ")
	...
	# Load Crime Syndicate recipes inside the bench
	var bench = get_node_or_null("CraftingBench")
	if bench:
		bench.bench_name = "Syndicate Desk"
		bench.recipes.clear()
