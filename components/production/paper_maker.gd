...
func _ready() -> void:
	...
	add_to_group("PaperMakers")
	add_to_group("MarketStall")
	add_to_group("nav_carve_obstacles")
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Paper Maker")
	...
	# Load standard Paper Maker recipe inside the bench
	var bench = get_node_or_null("CraftingBench")
	if bench:
		bench.bench_name = "Paper Maker"
		var make_paper = load("res://common/items/recipes/make_paper.tres")
		bench.recipes.clear()
		if make_paper:
			bench.recipes.append(make_paper)
