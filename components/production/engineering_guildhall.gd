...
func _ready() -> void:
	...
	add_to_group("EngineeringGuildhalls")
	add_to_group("MarketStall")
	add_to_group("nav_carve_obstacles")
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Civil Engineering Guildhall")
	...
	# Load Engineering Guildhall recipes inside the bench
	var bench = get_node_or_null("CraftingBench")
	if bench:
		bench.bench_name = "Guild Project Board"
		bench.recipes.clear()
		var r1 = load("res://common/items/recipes/contract_bridge_reconstruction.tres")
		var r2 = load("res://common/items/recipes/contract_palace_remodeling.tres")
...
func _process(delta: float) -> void:
	...
	if building_storage:
		_process_event_certificate(
			"bridge_reconstruction_contract",
			"res://common/items/instances/Finished Goods/bridge_reconstruction_contract.tres",
			"res://common/items/recipes/contract_bridge_reconstruction.tres",
			"Bridge Reconstruction Contract",
			"Garrison"
		)
		_process_event_certificate(
			"palace_remodeling_contract",
			"res://common/items/instances/Finished Goods/palace_remodeling_contract.tres",
			"res://common/items/recipes/contract_palace_remodeling.tres",
			"Palace Remodeling Contract",
			"Nobility"
		)
...
func _process_event_certificate(cert_id: String, cert_path: String, recipe_path: String, service_name: String, guest_type: String) -> void:
	...
	var base_influence: int = 30 if cert_id == "bridge_reconstruction_contract" else 50
	var base_prestige: int = 60 if cert_id == "bridge_reconstruction_contract" else 100
