...
func _ready() -> void:
	...
	add_to_group("MusicSalons")
	add_to_group("MarketStall")
	add_to_group("nav_carve_obstacles")
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Music Salon")
	...
	if bench:
		bench.bench_name = "Music Salon Bench"
		bench.recipes.clear()
		var recipes_to_load = [
			"res://common/items/recipes/craft_sheet_music.tres",
			"res://common/items/recipes/aristocratic_private_tutoring.tres",
			"res://common/items/recipes/chamber_concert_recital.tres"
		]
