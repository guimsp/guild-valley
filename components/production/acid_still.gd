...
func _ready() -> void:
	...
	add_to_group("AcidStills")
	add_to_group("MarketStall")
	add_to_group("nav_carve_obstacles")
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Acid Crucible & Still")
	...
	# Load Acid Crucible & Still recipes inside the bench
	var bench = get_node_or_null("CraftingBench")
	if bench:
		bench.bench_name = "Alchemical Crucible"
		bench.recipes.clear()
		var r1 = load("res://common/items/recipes/brew_alchemical_acid.tres")
		var r2 = load("res://common/items/recipes/refine_vitriol_solvent.tres")
