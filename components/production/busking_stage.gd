...
func _ready() -> void:
	...
	add_to_group("BuskingStages")
	add_to_group("MarketStall")
	add_to_group("nav_carve_obstacles")
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Busking Stage")
	...
	if bench:
		bench.bench_name = "Busking Stage Bench"
		bench.recipes.clear()
		var recipes_to_load = [
			"res://common/items/recipes/local_busking_service.tres"
		]
