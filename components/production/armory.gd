...
const UPGRADE_REQUIREMENTS: Dictionary = {
	2: { "gold_cost": 1400, "time": 20.0, "profession_level": 7 },
	3: { "gold_cost": 2500, "time": 30.0, "profession_level": 9 }
}

@export var max_concurrent_slots: int = 4
...
func _ready() -> void:
	...
	GameState.add_text_tag(self, custom_name if custom_name != "" else "The Armory")
...
func recalculate_building_parameters() -> void:
	var bench = get_node_or_null("CraftingBench")
	if bench:
		bench.bench_name = "Armorer's Bench"
		bench.recipes.clear()
		
		# Level 1 Recipes
		var r1 = load("res://common/items/recipes/forge_iron_shortsword.tres")
		var r2 = load("res://common/items/recipes/rivet_chainmail_padding.tres")
		var r3 = load("res://common/items/recipes/garrison_outfitting_service.tres")
		...
		# Level 2 Recipes
		if building_level >= 2:
			var r4 = load("res://common/items/recipes/forge_steel_broadsword.tres")
			var r5 = load("res://common/items/recipes/hammer_plate_breastplate.tres")
			if r4: bench.recipes.append(r4)
			if r5: bench.recipes.append(r5)
			
		# Level 3 Recipes
		if building_level >= 3:
			var r6 = load("res://common/items/recipes/craft_gilded_rapier.tres")
			var r7 = load("res://common/items/recipes/forge_ceremonial_armor.tres")
