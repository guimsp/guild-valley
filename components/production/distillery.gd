...
func _ready() -> void:
	...
	add_to_group("Distilleries")
	add_to_group("MarketStall")
	add_to_group("nav_carve_obstacles")
	GameState.add_text_tag(self, custom_name if custom_name != "" else ("Grand Distillery" if building_level >= 2 else "Distillery"))
	...
	# Load Distillery recipes inside the bench
	var bench = get_node_or_null("CraftingBench")
	if bench:
		bench.bench_name = "Distillery Tub"
		bench.recipes.clear()
		var r1 = load("res://common/items/recipes/ferment_wine.tres")
		var r2 = load("res://common/items/recipes/distill_moonshine.tres")
		var r3 = load("res://common/items/recipes/distill_schnapps.tres")
		if r1: bench.recipes.append(r1)
		if r2: bench.recipes.append(r2)
		if r3 and building_level >= 2: bench.recipes.append(r3)
