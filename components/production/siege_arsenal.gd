...
func _ready() -> void:
	...
	add_to_group("SiegeArsenals")
	add_to_group("MarketStall")
	add_to_group("nav_carve_obstacles")
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Imperial Siege Arsenal")
	...
	# Load Siege Arsenal recipes inside the bench
	var bench = get_node_or_null("CraftingBench")
	if bench:
		bench.bench_name = "Siege Workbench"
		bench.recipes.clear()
		var r1 = load("res://common/items/recipes/steel_arbalest_trigger.tres")
		var r2 = load("res://common/items/recipes/imperial_guard_halberd.tres")
		var r3 = load("res://common/items/recipes/iron_shod_defense_spikes.tres")
