...
func _ready() -> void:
	...
	add_to_group("ScholarsStudies")
	add_to_group("MarketStall")
	add_to_group("nav_carve_obstacles")
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Scholar's Study")
	...
	# Load Scholar's Study recipes inside the bench
	var bench = get_node_or_null("CraftingBench")
	if bench:
		bench.bench_name = "Scholar's Study Desk"
		bench.recipes.clear()
		
		var r_deed = load("res://common/items/recipes/draft_basic_deed.tres")
		var r_ledger = load("res://common/items/recipes/compile_registry_ledger.tres")
		var r_tome = load("res://common/items/recipes/bind_apprenticeship_tome.tres")
		var r_blueprint = load("res://common/items/recipes/draft_architectural_blueprint.tres")
		var r_folio = load("res://common/items/recipes/compile_masterwork_folio.tres")
		var r_passport = load("res://common/items/recipes/forge_regional_trade_passport.tres")
		
		if r_deed: bench.recipes.append(r_deed)
		if r_ledger: bench.recipes.append(r_ledger)
		if r_tome: bench.recipes.append(r_tome)
		if r_blueprint: bench.recipes.append(r_blueprint)
		if r_folio: bench.recipes.append(r_folio)
		if r_passport: bench.recipes.append(r_passport)
