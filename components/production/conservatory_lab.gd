...
func _ready() -> void:
	...
	add_to_group("Conservatories")
	add_to_group("MarketStall")
	add_to_group("nav_carve_obstacles")
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Conservatory Lab")
	...
	# Load Conservatory recipes inside the bench
	var bench = get_node_or_null("CraftingBench")
	if bench:
		bench.bench_name = "Catalyst Synthesizer"
		bench.recipes.clear()
		var r1 = load("res://common/items/recipes/extract_sulfur_pods.tres")
		var r2 = load("res://common/items/recipes/formulate_rejuvenation_tonic.tres")
		var r3 = load("res://common/items/recipes/extract_exotic_spices.tres")
		var r4 = load("res://common/items/recipes/synthesize_void_catalyst.tres")
