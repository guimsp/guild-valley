...
func _ready() -> void:
	...
	add_to_group("GreenhouseSpires")
	add_to_group("MarketStall")
	add_to_group("nav_carve_obstacles")
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Alchemical Greenhouse Spire")
	...
	# Load Greenhouse Spire recipes inside the bench
	var bench = get_node_or_null("CraftingBench")
	if bench:
		bench.bench_name = "Masterpiece Transmutation Rig"
		bench.recipes.clear()
		var r1 = load("res://common/items/recipes/distill_eternal_restlessness.tres")
		var r2 = load("res://common/items/recipes/transmute_philosophers_catalyst.tres")
