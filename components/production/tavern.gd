...
func _ready() -> void:
	...
	GameState.add_text_tag(self, custom_name if custom_name != "" else ("Casino" if building_level >= 2 else "Mead Tavern"))
	...
	# Load Tavern recipes inside the bench
	var bench = get_node_or_null("CraftingBench")
	if bench:
		bench.bench_name = "Tavern Counter"
		bench.recipes.clear()
