...
func _ready() -> void:
	...
	add_to_group("DryingSheds")
	add_to_group("MarketStall")
	add_to_group("nav_carve_obstacles")
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Biomass Drying Shed")
	...
	# Load Drying Shed recipes inside the bench
	var bench = get_node_or_null("CraftingBench")
	if bench:
		bench.bench_name = "Flora Press & Herb Dryer"
		bench.recipes.clear()
		var r1 = load("res://common/items/recipes/press_flora_oil.tres")
		var r2 = load("res://common/items/recipes/dehydrate_herbs.tres")
