...
func _ready() -> void:
	...
	add_to_group("SovereignMints")
	add_to_group("MarketStall")
	add_to_group("nav_carve_obstacles")
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Sovereign Mint & Library")
	...
	# Load Sovereign Mint recipes inside the bench
	var bench = get_node_or_null("CraftingBench")
	if bench:
		bench.bench_name = "Masterpiece Press"
		bench.recipes.clear()
		
		var r_charter = load("res://common/items/recipes/engrave_central_banking_charter.tres")
		var r_fiat = load("res://common/items/recipes/print_imperial_currency_decree.tres")
		
		if r_charter: bench.recipes.append(r_charter)
		if r_fiat: bench.recipes.append(r_fiat)
