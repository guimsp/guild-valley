...
func _ready() -> void:
	...
	add_to_group("InstrumentWorkshops")
	add_to_group("MarketStall")
	add_to_group("nav_carve_obstacles")
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Instrument Workshop")
	...
	if bench:
		bench.bench_name = "Instrument Workshop Bench"
		bench.recipes.clear()
		var recipes_to_load = [
			"res://common/items/recipes/draw_gut_strings.tres",
			"res://common/items/recipes/build_masterwork_lute.tres",
			"res://common/items/recipes/forge_brass_horn.tres"
		]
