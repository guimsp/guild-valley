...
func _ready() -> void:
	...
	add_to_group("Infirmaries")
	add_to_group("MarketStall")
	add_to_group("nav_carve_obstacles")
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Infusion Infirmary")
	...
	# Load Infirmary recipes inside the bench
	var bench = get_node_or_null("CraftingBench")
	if bench:
		bench.bench_name = "Infirmary Infusion Desk"
		bench.recipes.clear()
		var r1 = load("res://common/items/recipes/mix_antitoxin.tres")
		var r2 = load("res://common/items/recipes/local_apothecary_clinic.tres")
