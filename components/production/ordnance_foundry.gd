...
func _ready() -> void:
	...
	add_to_group("OrdnanceFoundries")
	add_to_group("MarketStall")
	add_to_group("nav_carve_obstacles")
	GameState.add_text_tag(self, custom_name if custom_name != "" else "Imperial Ordnance Foundry")
	...
	# Load Ordnance Foundry recipes inside the bench
	var bench = get_node_or_null("CraftingBench")
	if bench:
		bench.bench_name = "Foundry Project Board"
		bench.recipes.clear()
		var r1 = load("res://common/items/recipes/contract_ballista_fleet.tres")
		var r2 = load("res://common/items/recipes/contract_honor_guard.tres")
...
func _process(delta: float) -> void:
	...
	if building_storage:
		_process_event_certificate(
			"ballista_fleet_contract",
			"res://common/items/instances/Finished Goods/ballista_fleet_contract.tres",
			"res://common/items/recipes/contract_ballista_fleet.tres",
			"Ballista Fleet Contract",
			"Garrison"
		)
		_process_event_certificate(
			"honor_guard_uniform_contract",
			"res://common/items/instances/Finished Goods/honor_guard_uniform_contract.tres",
			"res://common/items/recipes/contract_honor_guard.tres",
...
func _process_event_certificate(cert_id: String, cert_path: String, recipe_path: String, service_name: String, guest_type: String) -> void:
	...
	var base_influence: int = 25 if cert_id == "ballista_fleet_contract" else 40
	var base_prestige: int = 50 if cert_id == "ballista_fleet_contract" else 80
