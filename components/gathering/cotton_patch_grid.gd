...
func _ready() -> void:
	...
	add_to_group("CottonPatchGrids")
	GameState.add_text_tag(self, "Cotton Patch")
...
func _spawn_crops() -> void:
	...
	var plant_scene = load("res://components/gathering/cotton_plant.tscn")
	if not plant_scene:
		return
		
	# Grid spacing is 32 pixels
	for r in range(4):
		for c in range(4):
			if r % 2 == 0:
				var plot = plant_scene.instantiate() as Node2D
				...
				soil.color = Color(0.24, 0.18, 0.12, 1.0)
				add_child(soil)
